...
<?php
	if (isset($_POST["ekle"]) || isset($_POST["kaydet"]) || isset($_POST["sirakaydet"])) {
		...
		if (isset($_POST["ekle"])) {
			...
			$ekle = mysql_query("INSERT INTO ustmenu (ustid, baslik, seo, url, harici, icon, tekaltmenu, bolge) values ('$ustid','$baslik','$seo','$url','$harici','$icon','$tekaltmenu','$bolge')");
		...
		if (isset($_POST["kaydet"])) {
			$kaydet = mysql_query("UPDATE ustmenu SET ustid = '$ustid', baslik = '$baslik', seo = '$seo', harici = '$harici', url = '$url', icon = '$icon', tekaltmenu = '$tekaltmenu', bolge = '$bolge' where id = '$id'");
		...
		if (isset($_POST["sirakaydet"])) {
			...
			for ($i=0; $i < count($siraid) ; $i++) {
				...
				$sirakaydet = mysql_query("UPDATE ustmenu SET sira = '$sira[$i]' where id = '$siraid[$i]'");
					  ...
					  <div class="col-sm-10">
						<?php
							...
							function ustmenu($id = 0, $string = 0, $ustid) {
							  $query = mysql_query("SELECT * FROM ustmenu WHERE ustid = '$id'");
...
<form action="" method="post" enctype="multipart/form-data">
	<?php
		$ustmenuler = mysql_query("SELECT * FROM ustmenu where id = '$id'");
		$u = mysql_fetch_array($ustmenuler);
	?>
	<section class="content">
		<div class="box">
			<div class="box-header with-border">
						...
						<select class="form-control select2" name="ustid">
							...
							<?php } else { ?>
								<?php
									...
									$ustidver = mysql_query("SELECT * FROM ustmenu where id = '".$u["ustid"]."'");
...
<section class="content">
	...
	<?php
		...
		if ($durum == "0") {
			$d = mysql_query("UPDATE ustmenu SET durum = '0' where id = '$id'");
			go("index.php?do=islem&icerik=ustmenu&islem=liste&hareket=onay&id=$id",0);
		}
		if ($durum == "1") {
			$d = mysql_query("UPDATE ustmenu SET durum = '1' where id = '$id'");
			go("index.php?do=islem&icerik=ustmenu&islem=liste&hareket=onay&id=$id",0);
		}

		if ($hareket == "sil") {

			$sil = mysql_query("DELETE FROM ustmenu where id = '$id'");
			$silustid = mysql_query("DELETE FROM ustmenu where ustid = '$id'");
				...
				<table id="example1" class="table table-bordered dataTable" colspan="5" role="grid" aria-describedby="example1_info">
				    ...
				    <tbody>
	                        ...
	                        function ustmenu($id = 0, $i = 0, $string = true) {
	                        	$anakat = mysql_query("SELECT * FROM ustmenu where ustid = '$id' order by sira ASC");
							...
							<tr>
								...
								<?php if ($akat["ustid"] != 0) { ?>
									<th>
										<?php
											$ustkat = mysql_query("SELECT * FROM ustmenu where id = '".$akat["ustid"]."'");

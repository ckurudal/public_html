<?php  
	...
	$bloglar = mysql_query("SELECT * FROM blog order by id DESC");
	$bloglarid = mysql_query("SELECT * FROM blog where id = '$id'");

	$blogkategori = mysql_query("SELECT * FROM blog_kategori order by id ASC");
...
<?php 
	if (isset($_POST["blogekle"]) || isset($_POST["blogkaydet"]) || isset($_POST["sirakaydet"])) {
		...
		if (isset($_POST["blogekle"])) {
			...
			$blogekle = mysql_query("INSERT INTO blog (baslik, icerik, seo, title, aciklama, keyw, kategori, video, sira, tarih) values ('$baslik','$icerik','$seo','$title','$aciklama','$keyw','$kategori','$video','$sira','$tarih')");	
				    ...
				    if (is_uploaded_file($_FILES["resim"]["tmp_name"][$i])) {
				        ...
				        if (in_array($resim_uzanti, $uzantilar)) {
				            ...
				            if (move_uploaded_file($_FILES["resim"]["tmp_name"][$i], $dosya)) {
				                ...
				                $ids = mysql_query("SELECT * FROM blog order by id desc limit 1");
				                $id = mysql_fetch_array($ids); 

				                $link = "/uploads/resim/".$saat.".jpg";
				                $ekle = mysql_query("UPDATE blog SET resim = '$link' where id = '".$id["id"]."'");
				                $yuklenenler++; 
		...
		if (isset($_POST["blogkaydet"])) {
			...
			$unvankaydet = mysql_query("UPDATE blog SET baslik = '$baslik', icerik = '$icerik', seo = '$seo', title = '$title', aciklama = '$aciklama', keyw = '$keyw', kategori = '$kategori', video = '$video' where id = '$id'");	
				    ...
				    if (is_uploaded_file($_FILES["resim"]["tmp_name"][$i])) {
				        ...
				        if (in_array($resim_uzanti, $uzantilar)) {
				            ...
				            if (move_uploaded_file($_FILES["resim"]["tmp_name"][$i], $dosya)) {
				            	...
				            	$resimal = mysql_query("SELECT * FROM blog where id = $id");
				            	$ral = mysql_fetch_array($resimal);

								$sil = @unlink("..".$ral['resim']);
				                
				                $link = "/uploads/resim/".$saat.".jpg";
				                $ekle = mysql_query("UPDATE blog SET resim = '$link' where id = '$id'");
				                $yuklenenler++; 
		...
		if (isset($_POST["sirakaydet"])) {
			...
			for ($i=0; $i < count($siraid) ; $i++) { 
				...
				$sirakaydet = mysql_query("UPDATE blog SET sira = '$sira[$i]' where id = '$siraid[$i]'");
					...
					<div class="form-group">
					  ...
					  <div class="col-sm-10"> 
						<select class="form-control select2" name="kategori">
							...
							<?php 
								while($sk = mysql_fetch_array($blogkategori)) {
							?> 
							<?php 
								if ($sk["durum"] == 0) {
							?>
				...
				<div class="form-horizontal">
					...
					<div class="form-group">
					  ...
					  <div class="col-sm-10"> 
						<select class="form-control select2" name="kategori">
							<?php
								$sfkatadi = mysql_query("SELECT * FROM blog_kategori Where id = '".$s["kategori"]."'");
...
<section class="content">
	<?php 
		...
		if ($hareket == "sil") {
			...
			$sil = mysql_query("DELETE FROM blog where id = '$id'"); 

			$ral = mysql_fetch_array($bloglarid); 
		...
		if ($durum == "0") {
			$d = mysql_query("UPDATE blog SET durum = '0' where id = '$id'"); 
			go("index.php?do=islem&icerik=blog&islem=liste&hareket=onay&id=$id",0);
		}
		if ($durum == "1") {
			$d = mysql_query("UPDATE blog SET durum = '1' where id = '$id'"); 
			go("index.php?do=islem&icerik=blog&islem=liste&hareket=onay&id=$id",0);
		}
	?>
		...
		<div class="bo x">	   	
			<div class="box-b ody table-responsive">
				...
				<table id="example1" class="table table-bordered dataTable" colspan="5" role="grid" aria-describedby="example1_info">
			    	...
			    	<tbody>
				    	<?php 
				    		while($sliste = mysql_fetch_array($bloglar)) {
				    	?>
			    		<tr>
			    			<th><?=$sliste["id"];?></th>
			    			<th>
			    			...
			    			<th>
			    				<?php 
			    					$blogkatadi = mysql_query("SELECT * FROM blog_kategori where id = '".$sliste["kategori"]."'");
			    					$sk = mysql_fetch_array($blogkatadi);
			    				?>
			    				<?=$sk["baslik"];?>
			    			</th>
			    			<th>
